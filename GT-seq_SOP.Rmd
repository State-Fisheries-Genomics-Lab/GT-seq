...
Note also that to run the GTseq example script using the example data, use the data found at example_data/ in the repository (you don't need to transfer the data off the server). It will has been cleaned up and modified to reflect the most current sequencing approach we use and will run smoothly, details are in the relevant sections of the SOP below.

Alternatively, copying the directory in [this repository](https://github.com/State-Fisheries-Genomics-Lab/GT-seq) named "example data" will provide all the relevant files to run the local portion (R) of the example protocol.

Make sure you load the required libraries (first code chunk in this notebook). 
...
The example dataset uses fall run and half-pounder steelhead from the Rogue River. Note that the data on the server can be used to run the full pipeline, but yo can also skip the server side analysis (genotype calling) and run just the local side analysis (filtering and data visualization) using the example data provided. 
...
Data came already demultiplexed from sequencing center. Location in code chunk below

```{bash, eval = FALSE}
# SERVER

/dfs/Omalley_Lab/dayan/gtseq_sop/raw_fastqs # 150 fastqs, 
```
...
Run fastqc report (not included in SOP) to get a look at this kind of information


## Demultiplex

The first step is to demultiplex the raw sequencing file using the i5 and i7 indexes. We can actually skip this step in the example script because the sequencing center already performed a demux. Instead, just copy the demuxed files to a working directory.  
...
FASTQS=(`ls /dfs/Omalley_Lab/dayan/gtseq_sop/raw_fastqs/*fastq`)
INFILE=${FASTQS[$SGE_TASK_ID -1]}
...
__-tc:__ this is the throttle. the value used in the script above (20), means we will submit the 554 separate commands in batches of 20 until it is done. you should check that this many nodes are available in the submit queue (use SGE_Avail)   
__submitting__: I usually store all of the scripts I will submit to the server in a directory named "sge," so if I wanted to submit this script (named gtseq_geno.txt) to thte otter queue, I'd submit the command qsub -q otter path/to/my/scripts/sge/gtseq_geno.txt  . you can check the progress with the qstat command.
...
At this point in the suggested published protocol, you should run the SummaryFigures script from the GTseq pipeline. However, this script contains harcoded values for genotype calling that are different from the actual genotyping script and will misrepresent the data. __Do not run it__, it will mislead you. We will produce the same figures in the next section.
